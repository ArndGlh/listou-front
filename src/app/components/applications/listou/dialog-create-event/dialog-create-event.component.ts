...
export class DialogCreateEventComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.createEventFormGroup = this.fb.group({
      ...
      location: ['A venir', [Validators.required]],
    });

    this.createEventFormGroup.controls['location'].disable();
  }
  ...
  public onAutocompleteSelected($event: any) {
    this.createEventFormGroup.controls.location.setValue(
      $event.formatted_address
    );
  }

  public changeDateEvent($event: any) {
    this.createEventFormGroup.controls.startDate.setValue(
